...
d3.csv("./updated_three_years.csv").then(function (data){
    ...
    function handleChange() {
        ...
        // filter data
        var filteredFacility = data.filter(d => d.AKA_name === facilityInputValue)
        var filteredLocation = data.filter(d => d.Address === locationInputValue)
        var filteredData = data.filter(d => d.AKA_name === facilityInputValue && d.Address === locationInputValue)
        console.log(filteredData);

        let response = {
            filteredData, filteredFacility, filteredLocation
        }
